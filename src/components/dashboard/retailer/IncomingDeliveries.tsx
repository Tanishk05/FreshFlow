...
const IncomingDeliveries: React.FC<Props> = ({ deliveries }) => {
  const [liveDeliveries, setLiveDeliveries] =
    useState<PurchaseOrder[]>(deliveries);
  ...
  useEffect(() => {
    ...
    const initSocket = async () => {
      try {
        ...
        socketInstance.on(
          "retailer-delivery-update",
          (update: { type: string; delivery: PurchaseOrder }) => {
            if (!isMounted) return;
            setLiveDeliveries((prev: PurchaseOrder[]) => {
              if (update.type === "add") {
                if (
                  !prev.some((d: PurchaseOrder) => d.id === update.delivery.id)
                ) {
                  return [update.delivery, ...prev];
                }
                return prev;
              } else if (update.type === "update") {
                return prev.map((d: PurchaseOrder) =>
                  d.id === update.delivery.id ? { ...d, ...update.delivery } : d
                );
              } else if (update.type === "remove") {
                return prev.filter(
                  (d: PurchaseOrder) => d.id !== update.delivery.id
                );
              }
              return prev;
            });
    ...
    return () => {
      isMounted = false;
      if (socketInstance) {
        socketInstance.off("retailer-delivery-update");
      }
    };
  }, []);
  // Check if any delivery has temperature issues
  const hasTemperatureAlerts = liveDeliveries.some(
    (d: PurchaseOrder) => d.liveTemperature > 4
  );
      ...
      {liveDeliveries.length === 0 ? (
        ...
        <>
          ...
          <div className="space-y-3">
            {liveDeliveries
              .slice(0, 5)
              .map((delivery: PurchaseOrder, idx: number) => {
